function [h,xx,yy] = pcolored(x,y,c)
%PCOLORED
%       PCOLORED(X,Y,C) is a modified version of PCOLOR that expands the 
...
%yL = y(:,1); yR = y(:,n);
% new
xL = 1.5*x(:,1)-0.5*x(:,2  ); xR = 1.5*x(:,n)-0.5*x(:,n-1);
yL = 1.5*y(:,1)-0.5*y(:,2  ); yR = 1.5*y(:,n)-0.5*y(:,n-1);

x = [ xL  0.5*(x(:,1:n-1) + x(:,2:n))  xR];
y = [ yL  0.5*(y(:,1:n-1) + y(:,2:n))  yR];

% old
%xB = x(1,:); xT = x(m,:);
%yB = y(1,:); yT = y(m,:);
% new
xB = 1.5*x(1,:)-0.5*x(2,:); xT = 1.5*x(m,:)-0.5*x(m-1,:);
yB = 1.5*y(1,:)-0.5*y(2,:); yT = 1.5*y(m,:)-0.5*y(m-1,:);

x = [ xB; 0.5*(x(1:m-1,:) + x(2:m,:)); xT];
y = [ yB; 0.5*(y(1:m-1,:) + y(2:m,:)); yT];
